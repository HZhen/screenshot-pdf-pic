...
export default {
  ...
  methods: {
    ...
    createCrop(e) {
      e.preventDefault();
      // 移动生成大小
      let nowX = 'clientX' in e ? e.clientX : e.touches ? e.touches[0].clientX : 0;
      let nowY = 'clientY' in e ? e.clientY : e.touches ? e.touches[0].clientY : 0;
      this.$nextTick(() => {
    ...
    goAutoCrop(cw, ch) {
      ...
      // 截图框默认大小
      // 如果为0 那么计算容器大小 默认为80%
      let w = cw ? cw : parseFloat(this.autoCropWidth);
      let h = ch ? ch : parseFloat(this.autoCropHeight);
      if (w === 0 || h === 0) {
        w = maxWidth * 0.8;
        h = maxHeight * 0.8;
      }
